...
#define TOLERANCE 100 // The maximum difference between the LDR readings to consider them equal
#define MAX_ITERATIONS 50 // The maximum number of iterations to perform in each axis
...
void move_x(int dir, bool limit) {

  // Check if the movement is within the limits or limit is false 
  if ((x_pos + dir >= X_MIN && x_pos + dir <= X_MAX) || !limit) {
...
void move_y(int dir, bool limit) {

  // Check if the movement is within the limits or limit is false 
  if ((y_pos + dir >= Y_MIN && y_pos + dir <= Y_MAX) || !limit) {
...
void home() {
   ...
   while (digitalRead(X_ENDSTOP) == HIGH && steps < (X_MAX - X_MIN) * STEPS_PER_REV / DEG_PER_STEP) {
    move_x(-1, false); // Pass false as limit argument
    steps++;
   }
   ...
   while (x_pos < (X_MIN + X_MAX) /2) {
    move_x(1, true); // Pass true as limit argument
   }
   ...
   while (digitalRead(Y_ENDSTOP) == HIGH && steps < (Y_MAX - Y_MIN) * STEPS_PER_REV / DEG_PER_STEP) {
    move_y(-1, false); // Pass false as limit argument
    steps++;
   }
   ...
   while (y_pos < (Y_MIN + Y_MAX) /2) {
    move_y(1, true); // Pass true as limit argument
   }
}
